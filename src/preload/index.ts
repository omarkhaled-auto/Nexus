...
const nexusAPI = {
   ...
   */
  onNexusEvent: (callback: (event: { type: string; payload: unknown; timestamp: Date }) => void): Unsubscribe => {
    const handler = (_event: IpcRendererEvent, data: { type: string; payload: unknown; timestamp: Date }): void => {
      callback(data)
    }
