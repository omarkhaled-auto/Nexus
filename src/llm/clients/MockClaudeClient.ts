// MockClaudeClient - Mock implementation for testing
...
/**
 * MockClaudeClient - Mock implementation of ClaudeClient for testing.
 *
 * Supports:
...
export class MockClaudeClient implements LLMClient {
  ...
  constructor(config: MockResponseConfig = {}) {
    this.config = {
      defaultResponse: 'Mock response from Claude',
      delay: 0,
      ...config,
    };
  }
  ...
  async chat(messages: Message[], options?: ChatOptions): Promise<LLMResponse> {
    ...
    let responseText = this.config.defaultResponse ?? 'Mock response';
