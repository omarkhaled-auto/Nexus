import { c as createLucideIcon, Z as useCosts, j as jsxRuntimeExports, J as Card, a as cn, K as CardHeader, _ as CardTitle, M as CardContent, r as reactExports, $ as clsx, a0 as getDefaultExportFromCjs, s as reactDomExports, a1 as React$3, a2 as useAgentMetrics, B as Bot, E as Eye, q as React$4, a3 as fo, R as RefreshCw, a4 as MessageSquare, C as CircleAlert, a5 as useTimeline, k as useNavigate, a6 as useIsMetricsLoading, a7 as useOverview, a8 as useMetricsStore, D as Button, I as Plus, S as Sparkles, a9 as FolderOpen, aa as Link, o as ChevronRight, N as Dialog, O as DialogContent, Q as DialogHeader, U as DialogTitle, V as DialogDescription, W as Input, Y as DialogFooter, L as LoaderCircle } from "./index-BAupzAA3.js";
import { F as FileText } from "./file-text-BCHv0mPv.js";
import { B as Bug, G as GitMerge, P as Play, a as Pause } from "./play-sZqI575X.js";
import { T as TestTubeDiagonal, C as CodeXml } from "./test-tube-diagonal-K-bUUbHW.js";
import { C as Circle } from "./circle-DOUwW_1z.js";
...
import { C as CircleCheck } from "./circle-check-5tnVLibw.js";
import { C as CircleX } from "./circle-x-CVuhPeYb.js";
import { Z as Zap } from "./zap-DNvUGNYA.js";
import { A as AnimatedPage } from "./AnimatedPage-DbdA76Gw.js";
import { C as Clock } from "./clock-BaSugHLB.js";
/**
...
const formatters = {
  // Era
  G: function(date2, token, localize) {
  ...
  // Year
  y: function(date2, token, localize) {
    if (token === "yo") {
      const signedYear = date2.getFullYear();
      const year = signedYear > 0 ? signedYear : 1 - signedYear;
  ...
  Y: function(date2, token, localize, options) {
  ...
  // Quarter
  Q: function(date2, token, localize) {
  ...
  // Month
  M: function(date2, token, localize) {
    const month = date2.getMonth();
    switch (token) {
      case "M":
      case "MM":
  ...
  // Stand-alone month
  L: function(date2, token, localize) {
    const month = date2.getMonth();
    switch (token) {
      case "L":
  ...
  // Local week of year
  w: function(date2, token, localize, options) {
  ...
  // ISO week of year
  I: function(date2, token, localize) {
  ...
  // Day of the month
  d: function(date2, token, localize) {
    if (token === "do") {
      return localize.ordinalNumber(date2.getDate(), { unit: "date" });
    }
  ...
  // Day of week
  E: function(date2, token, localize) {
  ...
  // Local day of week
  e: function(date2, token, localize, options) {
    const dayOfWeek = date2.getDay();
  ...
  c: function(date2, token, localize, options) {
    const dayOfWeek = date2.getDay();
    const localDayOfWeek = (dayOfWeek - options.weekStartsOn + 8) % 7 || 7;
    switch (token) {
      case "c":
  ...
  // AM or PM
  a: function(date2, token, localize) {
  ...
  // AM, PM, midnight, noon
  b: function(date2, token, localize) {
    const hours = date2.getHours();
  ...
  // Hour [1-12]
  h: function(date2, token, localize) {
    if (token === "ho") {
  ...
  H: function(date2, token, localize) {
    if (token === "Ho") {
      return localize.ordinalNumber(date2.getHours(), { unit: "hour" });
    }
    return lightFormatters.H(date2, token);
  },
  // Hour [0-11]
  K: function(date2, token, localize) {
  ...
  m: function(date2, token, localize) {
    if (token === "mo") {
      return localize.ordinalNumber(date2.getMinutes(), { unit: "minute" });
    }
    return lightFormatters.m(date2, token);
  },
  // Second
  s: function(date2, token, localize) {
    if (token === "so") {
      return localize.ordinalNumber(date2.getSeconds(), { unit: "second" });
    }
...
function format(date2, formatStr, options) {
  const defaultOptions = getDefaultOptions();
  const locale2 = defaultOptions.locale ?? enUS;
    ...
    try {
      ...
      if (Array.isArray(historicalProgressData)) {
        const transformedProgressData = historicalProgressData.map((point2) => {
          const p2 = point2;
          return {
            timestamp: p2.timestamp instanceof Date ? p2.timestamp : new Date(p2.timestamp),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4 py-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Input,
          {
            ...
            onChange: (e3) => {
              setCreateProjectName(e3.target.value);
            },
